...
l.addChild(d),f=f._char.getHeight()*f.scale,f>this.textHeight&&(this.textHeight=f);return l};BitmapFont.prototype.arrangeChars=function(b,a,l,g,e,f,d,c,z){null==g&&(g=-1);null==e&&(e="center");null==f&&(f="center");null==d&&(d=!0);null==c&&(c=!0);if(null==l||0==l.length||0==b&&0==a)return[];0>g&&(g*=-this.mSize);for(var m=[[]],k=!1,h={},q=0,B=0,p=0,x=0;!k;){x=g/this.mSize;B=b/x;p=a/x;m=[];m.push([]);if(this.mLineHeight<=p)for(var A=-1,C=-1,y=0,r=0,t=[],q=l.length,n=0;n<q;++n){var D=!1,v=l.charCodeAt(n),
u=this.getChar(v);if(v==BitmapFont.CHAR_NEWLINE||v==BitmapFont.CHAR_CARRIAGE_RETURN)D=!0;else if(null==u)console.log("[BitmapFont] Missing character: "+v);else{if(v==BitmapFont.CHAR_SPACE||v==BitmapFont.CHAR_TAB)A=n;c&&(y=u.getKerning(C)/1+y/1);h=new CharLocation(u);h._char=u;h.x=y/1+u.getXOffset()/1;h.y=r/1+u.getYOffset()/1;t.push(h);y+=u.getXAdvance()/1;C=v;if(h.x+Number(u.getWidth())>B){h=-1==A?1:n-A;t.splice(t.length-h,h);if(0==t.length)break;n-=h;D=!0}}if(n==q-1)m.push(t),k=!0;else if(D)if(m.push(t),
A==n&&t.pop(),r+2*this.mLineHeight<=p)t=[],y=0,r+=this.mLineHeight,C=A=-1;else break}d&&!k?(--g,m.length=0):k=!0}b=[];n=m.length;q=r+this.mLineHeight;r=0;f==VAlign.BOTTOM?r=p-q:f==VAlign.CENTER&&(r=(p-q)/2);this.previousWidth=[];for(f=0;f<n;++f)if(p=m[f],q=p.length,0!=q){a=0;h=p[p.length-1];h=h.x-h._char.getXOffset()/1+h._char.getXAdvance()/1;e==HAlign.RIGHT?a=B-h:e==HAlign.CENTER&&(a=(B-h)/2);for(l=this.width=0;l<q;++l)h=p[l],this.width+=h._char.getXAdvance()/1+h._char.getXOffset()/1+1,h.x=x*(h.x+
a),h.y=x*(h.y+r+(f-1)*z),h.scale=x,0<h._char.getWidth()&&0<h._char.getHeight()&&b.push(h),this.mCharLocationPool.push(h);this.previousWidth.push(this.width)}this.width=this.previousWidth[0];for(n=1;n<this.previousWidth.length;n++)this.previousWidth[n]>this.width&&(this.width=this.previousWidth[n]);return b};BitmapFont.prototype.getName=function(){return this.mName};BitmapFont.prototype.getSize=function(){return this.mSize};BitmapFont.prototype.getLineHeight=function(){return this.mLineHeight};BitmapFont.prototype.setLineHeight=
